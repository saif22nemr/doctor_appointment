...
@section('title')
	@if($action == 'create')
						@lang('user.create_employee')
					@else
						@lang('user.edit_employee')
					@endif
@endsection
@section('content')
						...
						<div class="row">
							<div class="col-md-6 mb-3">
								...
								<input type="text" class="form-control" name="name" name="name" id="name" placeholder="@lang('app.entry_name')"
								 aria-describedby="inputGroupPrepend3" required value="{{$action == 'edit' ? $employee->name : old('name')}}">
							...
							<div class="col-md-6 mb-3">
								...
								<input type="text" class="form-control" title="username" name="username" id="username" placeholder="@lang('app.entry_username')"
								 aria-describedby="inputGroupPrepend3" required value="{{$action == 'edit' ? $employee->username : old('username')}}">
								<div class="invalid-feedback">
									@lang('app.error_general_form')
								</div>
							</div>
							<div class="col-md-6 mb-3">
								...
								<input type="email" class="form-control" title="email" name="email" id="email" placeholder="@lang('app.entry_email')"
								 aria-describedby="inputGroupPrepend3"  value="{{$action == 'edit' ? $employee->email : old('email')}}">
								<div class="invalid-feedback">
									@lang('app.error_general_form')
								</div>
							</div>
							...
							<div class="col-md-6 mb-3">
								...
								<select class="select2 form-control" id="status" name="status">
									<option value="1" {{($action == 'edit' and $employee->status == 1) ? 'selected' : ''}}>@lang('app.yes')</option>
									<option value="0" {{($action == 'edit' and $employee->status == 0) ? 'selected' : ''}}>@lang('app.no')</option>
								</select>
								<div class="invalid-feedback">
									@lang('app.error_general_form')
								</div>
							...
							<div class="col-md-6 mb-3">
								...
								<select class="select2 form-control" name="branch_id">
									@foreach($branchs as $branch)
										<option value="{{$branch->id}}" {{($action == 'edit' and $employee->branch_id = $branch->id) ? 'selected' :''}}>{{$branch->name}}</option>
									@endforeach
								</select>
	...
	function createAndEdit(xdata , type = 'create' , url = "" ){
		...
		$.ajax({
			...
			success: function(json){
				console.log(json);
				if(json.success){
					toastr.success( json.message, '@lang("app.employees")');	
	...
	$(function(){
		...
		$('#save').on('click' , function(){
			...
			if(Object.keys(formData).length > 0){
				var url = (action == 'edit') ? "{{url('api')}}/employee/{{$employee->id ?? ''}}" : "{{url('api')}}/employee"
